...
export const PlaygroundStateProvider = ({
  ...
  useEffect(() => {
    // Check for OpenAI key availability from the server
    const checkServerKey = async () => {
      try {
        ...
        if (data.hasKey) {
          // If server has the key, we set a placeholder value
          // The actual key remains secure on the server
          dispatch({ type: "SET_API_KEY", payload: "server_side_key" });
        }
      } catch (error) {
        console.error('Failed to verify OpenAI key:', error);
      }
    };
